...
# ============================================================================
#   GÉNÉRATION SYNTHÉTIQUE DES DONNÉES ÉNERGÉTIQUES + CHARGEMENT EN DB
# ============================================================================
...
def generate_realistic_temperature(
    hours, location_offset=0, building_type="House", building_id=0
):
    ...
    for hour in range(hours):
        ...
        if building_type == "House":
            seasonal_temp = (17.5 + location_offset) + (base_amplitude * amplitude_factor) * np.sin(
                2 * np.pi * (day_of_year - 80 + phase_shift) / 365
            )
        elif building_type == "Hospital":
            seasonal_temp = (18 + location_offset) + (20 * amplitude_factor) * np.sin(
                2 * np.pi * (day_of_year - 80 + phase_shift) / 365
            )
        elif building_type == "Industry":
            seasonal_temp = (17 + location_offset) + (21 * amplitude_factor) * np.sin(
                2 * np.pi * (day_of_year - 80 + phase_shift) / 365
            )
        elif building_type == "Office":
            seasonal_temp = (18 + location_offset) + (22 * amplitude_factor) * np.sin(
                2 * np.pi * (day_of_year - 80 + phase_shift) / 365
            )
        else:
            seasonal_temp = (17.5 + location_offset) + (22 * amplitude_factor) * np.sin(
                2 * np.pi * (day_of_year - 80 + phase_shift) / 365
            )
...
def add_temporal_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["IsHoliday"] = (
        ...
        | (
            (df["Time"].dt.month == 11)
            & (df["Time"].dt.day.between(22, 28))
        )
        | (
            (df["Time"].dt.month == 3)
            & (df["Time"].dt.day.between(15, 22))
        )
    ).astype(int)
...
def generate_building_data(building_type, building_number):
    ...
    temperatures = generate_realistic_temperature(
        hours_per_year, location_offset, building_type, building_number
    )
    ...
    df = pd.DataFrame(
        {
            "Time": pd.date_range(
                start="2023-01-01", periods=hours_per_year, freq="h"  # Minuscule
            ),
            "Winter": [
                1 if (i // (hours_per_year // 4)) % 4 == 0 else 0
                for i in range(hours_per_year)
            ],
            "Spring": [
                1 if (i // (hours_per_year // 4)) % 4 == 1 else 0
                for i in range(hours_per_year)
            ],
            "Summer": [
                1 if (i // (hours_per_year // 4)) % 4 == 2 else 0
                for i in range(hours_per_year)
            ],
            "Fall": [
                1 if (i // (hours_per_year // 4)) % 4 == 3 else 0
                for i in range(hours_per_year)
            ],
    ...
    occupancy_internal = generate_realistic_occupancy(
        df, building_type, building_size_factor, building_number
    )

    df["Lighting [kW]"] = generate_realistic_lighting(
        df, building_type, cloud_cover, solar_radiation, building_efficiency, occupancy_internal
    )
    df["HVAC [kW]"] = generate_realistic_hvac(
        df, building_type, temperatures, solar_radiation, building_insulation, occupancy_internal
    )
...
def main():
    ...
    for building_type, count in building_counts.items():
        ...
        for i in range(1, count + 1):
            ...
            print(
                f"    Jan: {monthly_stats[1]:6.1f} | "
                f"Feb: {monthly_stats[2]:6.1f} | "
                f"Jun: {monthly_stats[6]:6.1f} | "
                f"Sep: {monthly_stats[9]:6.1f} | "
                f"Dec: {monthly_stats[12]:6.1f}"
            )
    ...
    # On enregistre dans le dossier data/ à la racine du projet
    base_dir = Path(__file__).resolve().parents[3]  # .../renewstation-huawei-cloud
    ...
    # 🔁 Charge dans la DB (table energy_consumption_hourly)
    load_energy_consumption_to_db(combined_df)

    print("\n" + "=" * 80)
    print("✅ SUCCÈS! Dataset avec PATTERNS UNIQUES pour chaque bâtiment")
