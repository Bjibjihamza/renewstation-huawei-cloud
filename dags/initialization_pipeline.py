...
def initialize_battery_states(**context):
    ...
    for typ, cap, uid in batteries:
        cur.execute("""
            ...
            ON CONFLICT (id) DO UPDATE SET soc_start_pct = 100.0, soc_end_pct = 100.0, energy_stored_kwh = %s
...
def mark_initialization_complete(**context):
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS pipeline_metadata (
            key VARCHAR(100) PRIMARY KEY, value TEXT, updated_at TIMESTAMP DEFAULT NOW()
        )
    """)
    cur.execute("""
        INSERT INTO pipeline_metadata (key, value) VALUES ('initialization_complete', 'true')
...
with DAG(
    ...
    # FLUX CORRIGÉ : weather → energy → batteries → done
